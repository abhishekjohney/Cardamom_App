...
class PartyListService {
  ...
  Future<PartyListResponse> getPartyList({
    String route = "",
    String groups =
        "Sundry Creditors", // Changed to match working React payload
  }) async {
    try {
      // Get current year
      ...
      print(
          '🔄 UPDATED SERVICE: Fetching party list with route: "$route", groups: "$groups"');
      ...
      if (response.statusCode == 200) {
        ...
        print(
            '📄 Raw Response (first 500 chars): ${responseBody.length > 500 ? responseBody.substring(0, 500) + "..." : responseBody}');

        // Remove the marker if present
        final endMarkerPos = responseBody.indexOf('||JasonEnd');
        if (endMarkerPos != -1) {
          print(
              '✂️ Found JasonEnd marker at position $endMarkerPos, trimming response');
        ...
        try {
          ...
          if (userData is Map && userData.containsKey('userdata')) {
            ...
            if (userDataObj is Map && userDataObj.containsKey('PartyList')) {
              ...
              print(
                  '✅ Found userdata.PartyList structure with ${partyList.length} parties');
          ...
          // Fallback: Check for direct PartyList structure
            ...
            print(
                '✅ Found direct PartyList structure with ${partyList.length} parties');
            return PartyListResponse.fromJson(
  ...
  Future<Map<String, dynamic>> getPartyTemplate() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception(
            'Failed to get party template: HTTP ${response.statusCode}');
      }
    } catch (e) {
  ...
  Future<List<Map<String, dynamic>>> getPartyPaymentDetails({
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        print(
            '📄 Payment Details Response (first 1000 chars): ${responseBody.length > 1000 ? responseBody.substring(0, 1000) + "..." : responseBody}');
        ...
        if (responseBody.trim().toLowerCase().startsWith('<!doctype html') ||
          ...
          throw Exception(
              'Authentication session issue: Server returned HTML login page instead of JSON data');
        }
        ...
        try {
          ...
          throw Exception(
              'Failed to parse payment details response: $jsonError');
        }
      } else if (response.statusCode == 302) {
        print('❌ 302 Redirect - authentication session expired or invalid');
        throw Exception(
            'Session expired: Server returned 302 redirect to login page');
      } else {
        print('❌ HTTP Error: ${response.statusCode}');
        print('📄 Error Response: ${response.data}');
        throw Exception('HTTP Error: ${response.statusCode}');
